...
def ensure_database_exists(user, pw, port, db_name):
    ...
    for attempt in range(retries):
        try:
            ...
            time.sleep(delay)
    raise RuntimeError("PostgreSQL is up but not ready for SQL connections after waiting.")

class TestNetCDFConversion(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def setUp(self):
        config_path = os.path.join(ROOT_DIR, "tests", "integration", "netcdf_conversion_config.yaml")
